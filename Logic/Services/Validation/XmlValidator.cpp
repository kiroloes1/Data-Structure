...
// HELPER: Extract Tags
// ---------------------------------------------------------
bool XmlValidator::parseTags(const std::string &xmlContent,
                             std::vector<Tag> &tags, std::string &errorLog) {
  ...
  for (size_t i = 0; i < xmlContent.length(); ++i) {
    ...
    if (c == '<') {
      if (insideTag) {
        errorLog = "Line " + std::to_string(currentLine) +
                   ": Unclosed tag bracket found (nested '<').";
        return false;
      }
      insideTag = true;
      buffer = "<";
      ...
      if (insideTag) {
        ...
        if (buffer.length() > 2 && buffer[1] == '/') {
          ...
          if (spacePos != std::string::npos)
            t.name = buffer.substr(1, spacePos - 1);
          else
            t.name = buffer.substr(1, buffer.length() - 2);
        }
        tags.push_back(t);
      }
    } else if (insideTag) {
...
bool XmlValidator::validate(const std::string &xmlContent,
                            std::string &errorLog) {
  std::vector<Tag> tags;
  if (!parseTags(xmlContent, tags, errorLog)) {
    return false;
  }
  ...
  for (const auto &tag : tags) {
    if (!tag.isClosing) {
      ...
      if (tagStack.empty()) {
        ss << "Line " << tag.lineIndex << ": Found closing tag </" << tag.name
           << "> but stack is empty.\n";
        errorCount++;
      } else {
        Tag top = tagStack.top();
        if (top.name != tag.name) {
          ss << "Line " << tag.lineIndex << ": Mismatched tag. Expected </"
             << top.name << "> but found </" << tag.name << ">.\n";
          errorCount++;
        } else {
          tagStack.pop();
        }
  ...
  while (!tagStack.empty()) {
    Tag t = tagStack.top();
    ss << "Line " << t.lineIndex << ": Tag <" << t.name
...
std::string XmlValidator::fix(const std::string &xmlContent) {
  ...
  for (size_t i = 0; i < xmlContent.length(); ++i) {
    ...
    if (c == '<') {
      ...
      if (j < xmlContent.length())
        tagStr += ">";

      bool isClosing = (tagStr.length() > 2 && tagStr[1] == '/');
      std::string name;
      if (isClosing)
        name = tagStr.substr(2, tagStr.length() - 3);
      else {
        size_t sp = tagStr.find(' ');
        if (sp != std::string::npos)
          name = tagStr.substr(1, sp - 1);
        else
          name = tagStr.substr(1, tagStr.length() - 2);
      }
